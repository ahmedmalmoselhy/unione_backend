...
@section('title', 'Assign Faculty Administrator — ' . $faculty->name)
...
<nav class="flex items-center gap-2 text-sm mb-6">
    ...
    <a href="{{ route('dashboard.faculties.show', $faculty) }}" class="text-gray-400 hover:text-gray-700 transition-colors">{{ $faculty->name }}</a>
    <svg class="w-3.5 h-3.5 text-gray-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
    </svg>
    <span class="text-gray-700 font-medium">Assign Administrator</span>
</nav>
...
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Current Faculty Administrator</h3>
    ...
    @if($currentAdmin)
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-medium text-gray-900">{{ $currentAdmin->first_name }} {{ $currentAdmin->last_name }}</p>
                    <p class="text-xs text-gray-400">{{ $currentAdmin->email }}</p>
                </div>
            </div>
            <form method="POST" action="{{ route('dashboard.faculties.assign-admin.revoke', $faculty) }}"
                  onsubmit="return confirm('Revoke faculty administrator access for {{ addslashes($currentAdmin->first_name . ' ' . $currentAdmin->last_name) }}?')">
                @csrf
                @method('DELETE')
                <button type="submit"
    ...
    @else
        <p class="text-sm text-gray-400">No faculty administrator currently assigned.</p>
    @endif
</div>
...
<div class="bg-white rounded-2xl border border-gray-200 p-6">
    ...
    @if($employees->isEmpty())
        <p class="text-sm text-gray-400">No employees found in this faculty's departments. Add employees first.</p>
    @else
        <form method="POST" action="{{ route('dashboard.faculties.assign-admin.store', $faculty) }}" class="space-y-4">
            @csrf

            <div>
                <label for="employee_user_id" class="block text-sm font-medium text-gray-700 mb-1.5">
                    Select Employee
                </label>
                <select name="employee_user_id" id="employee_user_id" required
                        class="w-full px-3 py-2 text-sm rounded-lg border {{ $errors->has('employee_user_id') ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-white' }} focus:outline-none focus:ring-2 focus:ring-blue-500 max-w-md">
            ...
            <button type="submit"
                ...
                Assign as Faculty Administrator
            </button>
        </form>
    @endif
</div>
