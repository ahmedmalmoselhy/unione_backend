...
<div class="max-w-2xl">
    ...
    <nav class="flex items-center gap-2 text-sm mb-6">
        ...
        <span class="text-gray-700 font-medium truncate">
            {{ $vice_president->professor->user->first_name }} {{ $vice_president->professor->user->last_name }}
        </span>
    </nav>

    <div class="bg-white rounded-2xl border border-gray-200 p-6">

        <form method="POST" action="{{ route('dashboard.university.vice-presidents.update', $vice_president) }}" class="space-y-5">
            @csrf
            @method('PUT')

            @include('dashboard.university.vice-presidents._form', ['vicePresident' => $vice_president])
