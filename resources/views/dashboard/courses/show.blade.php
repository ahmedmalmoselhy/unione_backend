...
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.courses.index') }}" class="text-gray-400 hover:text-gray-700 transition-colors">{{ __('courses.title') }}</a>
...
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5 text-sm">
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('courses.code') }}</dt>
            <dd><span class="font-mono text-sm bg-gray-100 text-gray-700 px-2 py-0.5 rounded">{{ $course->code }}</span></dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('courses.level') }}</dt>
            <dd class="text-gray-700">{{ __('courses.level_n', ['n' => $course->level]) }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('courses.credit_hours') }}</dt>
            <dd class="text-gray-700">{{ $course->credit_hours }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('courses.lecture_hours') }}</dt>
            <dd class="text-gray-700">{{ $course->lecture_hours }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('courses.lab_hours') }}</dt>
            <dd class="text-gray-700">{{ $course->lab_hours }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('courses.description') }}</dt>
...
{{-- Departments --}}
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{{ __('courses.departments') }}</h3>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{{ __('courses.prerequisites') }}</h3>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{{ __('courses.required_by') }}</h3>
...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
    ...
    @if($course->sections->isEmpty())
        <div class="px-6 py-10 text-center text-sm text-gray-400">{{ __('courses.no_sections') }}</div>
    @else
