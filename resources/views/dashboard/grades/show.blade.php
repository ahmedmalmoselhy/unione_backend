...
@section('heading', 'Grade Details')

@section('content')

{{-- Breadcrumb --}}
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.grades.index') }}" class="text-gray-400 hover:text-gray-700 transition-colors">Grades</a>
...
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5 text-sm mb-6">
        ...
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Enrollment Status</dt>
            <dd class="text-gray-700">{{ ucfirst($grade->enrollment?->status) }}</dd>
        </div>
    </dl>
    ...
    <div class="border-t border-gray-100 pt-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-5">
            <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Midterm</p>
                <p class="text-2xl font-bold text-gray-900">{{ $grade->midterm ?? '—' }}</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Coursework</p>
                <p class="text-2xl font-bold text-gray-900">{{ $grade->coursework ?? '—' }}</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Final</p>
                <p class="text-2xl font-bold text-gray-900">{{ $grade->final ?? '—' }}</p>
            </div>
            <div class="bg-blue-50 rounded-xl p-4 text-center">
                <p class="text-xs text-blue-500 uppercase tracking-wider mb-1">Total</p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-5 mt-5">
            <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Letter Grade</p>
                <p class="text-2xl font-bold text-gray-900">{{ $grade->letter_grade ?? '—' }}</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Grade Points</p>
                <p class="text-2xl font-bold text-gray-900">{{ $grade->grade_points ?? '—' }}</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Graded By</p>
