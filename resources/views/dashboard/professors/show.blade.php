...
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.professors.index') }}" class="text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">{{ __('professors.title') }}</a>
    ...
    <span class="text-gray-700 dark:text-gray-300 font-medium truncate">{{ $professor->user->first_name }} {{ $professor->user->last_name }}</span>
</nav>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 mb-8">
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            ...
            @endif
            <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ $professor->user->first_name }} {{ $professor->user->last_name }}</h2>
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5 text-sm">
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.staff_number_full') }}</dt>
            <dd><span class="font-mono text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded">{{ $professor->staff_number }}</span></dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('common.email') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->user->email }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.phone') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->user->phone ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.national_id') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300 font-mono text-xs">{{ $professor->user->national_id }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.gender') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ ucfirst($professor->user->gender) }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.date_of_birth') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->user->date_of_birth?->format('M d, Y') ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('common.department') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->department->name }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('common.faculty') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->department->faculty?->name ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.office_location') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->office_location ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('professors.hire_date') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $professor->hired_at->format('M d, Y') }}</dd>
        </div>
    </dl>
                ...
                @foreach($professor->sections as $section)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-5 py-3.5">
                            <div class="font-medium text-gray-900 dark:text-white">{{ $section->course->name ?? '—' }}</div>
                            <div class="text-xs text-gray-400 mt-0.5 font-mono">{{ $section->course->code ?? '' }}</div>
                        </td>
                        <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400">{{ $section->academic_year }}</td>
                        <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400">{{ $section->semester }}</td>
