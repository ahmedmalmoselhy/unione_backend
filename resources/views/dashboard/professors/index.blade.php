...
<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-gray-500">{{ $professors->total() }} {{ Str::plural(__('common.professor'), $professors->total()) }}</p>
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full max-w-lg rounded-2xl shadow-xl p-6 z-10">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-base font-semibold text-gray-900">{{ __('professors.import_professors') }}</h3>
            ...
            <form action="{{ route('dashboard.professors.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center mb-4">
                    ...
                    <p id="professors-import-filename" class="text-xs text-gray-400 mt-1">{{ __('professors.file_hint') }}</p>
                </div>
            ...
            <tbody class="divide-y divide-gray-50">
                @foreach($professors as $professor)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-5 py-3.5">
                            <div class="text-gray-700">{{ $professor->department->local_name }}</div>
                            <div class="text-xs text-gray-400 mt-0.5">{{ $professor->department->faculty?->local_name }}</div>
                        </td>
