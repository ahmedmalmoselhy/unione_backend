...
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.students.index') }}" class="text-gray-400 hover:text-gray-700 transition-colors">{{ __('students.title') }}</a>
...
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5 text-sm">
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.faculty') }}</dt>
            <dd class="text-gray-700">{{ $student->faculty?->name }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.department') }}</dt>
            <dd class="text-gray-700">{{ $student->department?->name ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.academic_year') }}</dt>
            <dd class="text-gray-700">Year {{ $student->academic_year }} · {{ ucfirst($student->semester) }} Semester</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.gpa') }}</dt>
            <dd class="text-gray-700">{{ $student->gpa ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.enrolled_at') }}</dt>
            <dd class="text-gray-700">{{ $student->enrolled_at?->format('M d, Y') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.graduated_at') }}</dt>
            <dd class="text-gray-700">{{ $student->graduated_at?->format('M d, Y') ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.gender') }}</dt>
            <dd class="text-gray-700">{{ ucfirst($student->user->gender) }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.phone') }}</dt>
            <dd class="text-gray-700">{{ $student->user->phone ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('students.account_status') }}</dt>
            <dd>
...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6">
    ...
    @if($student->enrollments->isEmpty())
        <div class="px-6 py-10 text-center text-sm text-gray-400">{{ __('students.no_enrollments') }}</div>
    @else
...
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{{ __('students.transfer_department') }}</h3>
    ...
    <form method="POST" action="{{ route('dashboard.students.transfer', $student) }}" class="flex flex-col sm:flex-row gap-3 items-end">
        @csrf
        <div class="flex-1">
            <label for="to_department_id" class="block text-xs font-medium text-gray-600 mb-1.5">{{ __('students.new_department') }}</label>
        ...
        <div class="flex-1">
            <label for="note" class="block text-xs font-medium text-gray-600 mb-1.5">{{ __('students.note') }} <span class="text-gray-400">({{ __('common.optional') }})</span></label>
