...
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.academic-terms.index') }}" class="text-gray-400 hover:text-gray-700 transition-colors">{{ __('academic_terms.title') }}</a>
...
<div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5 text-sm">
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.academic_year') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->academic_year }}/{{ $academicTerm->academic_year + 1 }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.semester_start') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->starts_at->format('M d, Y') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.semester_end') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->ends_at->format('M d, Y') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.registration_opens') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->registration_starts_at->format('M d, Y') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.registration_closes') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->registration_ends_at->format('M d, Y') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.withdrawal_deadline') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->withdrawal_deadline?->format('M d, Y') ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.exam_period_start') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->exam_starts_at?->format('M d, Y') ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.exam_period_end') }}</dt>
            <dd class="text-gray-700">{{ $academicTerm->exam_ends_at?->format('M d, Y') ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">{{ __('academic_terms.grade_submission_deadline') }}</dt>
...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
    ...
    @if($academicTerm->sections->isEmpty())
        <div class="px-6 py-10 text-center text-sm text-gray-400">{{ __('academic_terms.no_sections_yet') }}</div>
    @else
