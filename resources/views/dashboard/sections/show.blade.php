...
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.sections.index') }}" class="text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">{{ __('sections.title') }}</a>
    ...
    <span class="text-gray-700 dark:text-gray-300 font-medium truncate">{{ $section->course->code }} #{{ $section->id }}</span>
</nav>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ $section->course->name }}</h2>
            <p class="text-sm text-gray-500 mt-0.5">
                <span class="font-mono bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded text-xs">{{ $section->course->code }}</span>
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5 text-sm">
        <div>
            ...
            <dd class="text-gray-700 dark:text-gray-300">{{ $section->professor?->user?->first_name }} {{ $section->professor?->user?->last_name }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('sections.academic_term') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $section->academicTerm?->name ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('sections.capacity') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $section->capacity }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('sections.room') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $section->room ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('sections.enrolled') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $section->enrollments->count() }} / {{ $section->capacity }}</dd>
        </div>
    </dl>
</div>
...
@if($section->schedule && count($section->schedule))
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">{{ __('sections.schedule') }}</h3>
        <div class="flex flex-wrap gap-2">
            @foreach($section->schedule as $slot)
                <div class="flex items-center gap-2 px-3 py-2 rounded-lg border {{ ($slot['type'] ?? 'lecture') === 'lab' ? 'border-purple-200 bg-purple-50' : 'border-blue-200 bg-blue-50' }}">
                    ...
                    <span class="text-xs text-gray-600 dark:text-gray-400">{{ $slot['start_time'] ?? '' }} – {{ $slot['end_time'] ?? '' }}</span>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
    ...
    @if($section->enrollments->isEmpty())
        <div class="px-6 py-10 text-center text-sm text-gray-400 dark:text-gray-500">{{ __('sections.no_students_enrolled') }}</div>
    @else
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                @foreach($section->enrollments as $enrollment)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-5 py-3 text-gray-600 dark:text-gray-400">{{ $enrollment->student?->user?->email }}</td>
                        <td class="px-5 py-3">
