...
<nav class="flex items-center gap-2 text-sm mb-6">
    <a href="{{ route('dashboard.announcements.index') }}" class="text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">{{ __('announcements.title') }}</a>
    ...
    <span class="text-gray-700 dark:text-gray-300 font-medium truncate">{{ Str::limit($announcement->title, 50) }}</span>
</nav>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-4 gap-x-8 gap-y-4 text-sm mb-6">
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('announcements.author') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $announcement->author?->first_name }} {{ $announcement->author?->last_name }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('announcements.visibility') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $visLabels[$announcement->visibility] ?? ucfirst($announcement->visibility) }}{{ $announcement->target_id ? ' #' . $announcement->target_id : '' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('announcements.published') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $announcement->published_at?->format('M d, Y h:i A') ?? __('announcements.draft') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('announcements.expires_at') }}</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $announcement->expires_at?->format('M d, Y h:i A') ?? '—' }}</dd>
        </div>
    </dl>

    <div class="border-t border-gray-100 dark:border-gray-700 pt-5">
        <div class="prose prose-sm max-w-none text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $announcement->body }}</div>
    </div>
</div>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
    ...
    @if($announcement->reads->isEmpty())
        <div class="px-6 py-10 text-center text-sm text-gray-400 dark:text-gray-500">{{ __('announcements.no_reads') }}</div>
    @else
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                @foreach($announcement->reads as $read)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">{{ $read->read_at?->format('M d, Y h:i A') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
